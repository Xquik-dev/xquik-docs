...
<Steps>
  ...
  <Step title="Register a webhook">
    ...
    Provide an HTTPS URL and select which event types to receive. Xquik generates a signing secret. Store it securely. The API returns it only once.
...
After the 10th failed attempt, the delivery is marked as `exhausted`. A `410 Gone` response exhausts the delivery immediately. Check delivery status with the [deliveries endpoint](/api-reference/webhooks/deliveries).
...
Xquik processes retries in batches, so the actual delay may be slightly longer than shown.
