...
The receipt confirms the payment was settled and includes a reference ID and timestamp. The header is attached to every settled response (200, 4xx, 5xx) for both charge and session intents. Charge settlement is irreversible once broadcast on Tempo. Session voucher state has already advanced server-side. Always reconcile against the receipt on every response, including errors.
