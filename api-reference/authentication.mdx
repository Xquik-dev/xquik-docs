...
16 X-API read-only endpoints also accept [MPP](/mpp/overview) payments instead of API key authentication. When you call an eligible endpoint without an API key or session cookie, the server returns a 402 payment challenge.
...
See the [MPP overview](/mpp/overview#eligible-endpoints) for the full list of 16 endpoints, pricing, and intent types.
