...
The receipt confirms the payment was settled and includes a reference ID and timestamp. For charge-intent endpoints, the header is attached to every settled response (200, 4xx, 5xx) since Tempo settlement is irreversible once broadcast. Session-intent payments only return the receipt on 200.
