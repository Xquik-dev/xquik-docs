...
description: "List up to 200 webhook endpoints with URLs, event filters, active states, delivery status, failure counts, and creation times. The signing secret never appears."
api: "GET /webhooks"
...
These examples build one row per webhook. They do not print the full
...
to check which signed webhook endpoints the account has configured.
...
| Delivery status          | `webhooks[].deliveryStatus`      | Alert the owner on `needs_attention`.           |
| Consecutive failures     | `webhooks[].consecutiveFailures` | Alert before the receiver reaches its hard cap. |
| Failure cap              | `webhooks[].failureHardCap`      | Record the `needs_attention` threshold.         |
| Creation time            | `webhooks[].createdAt`           | Track how old each configuration is.            |
...
<CardGroup cols={2}>
  ...
  <Card title="Delivery status" icon="radio">
    ...
    Store `webhooks[].deliveryStatus`. Alert the receiver owner on
    `needs_attention`. Xquik keeps retrying. After the fix, call
    [Resume Webhook](/api-reference/webhooks/resume) to start sending waiting
    deliveries at once.
  ...
  <Card title="Failure counter" icon="activity">
    ...
    alerting before the receiver reaches the failure cap.
  ...
  <Card title="Signing secret" icon="shield-check">
    ...
    The list omits the signing `secret`. Store it from
