...
Set to `false` to pause deliveries. Set to `true` to turn it on and start
sending waiting deliveries at once. Either value resets `consecutiveFailures`
to `0`. To require a signed test first, use
...
Failure count that sets `needs_attention`. Delivery continues past it.
...
<CardGroup cols={2}>
  ...
  <Card title="Pause delivery" icon="toggle-left">
    ...
    delivery records remain available. Xquik keeps waiting deliveries for 30
  ...
  <Card title="Resume delivery" icon="toggle-right">
    ...
    `isActive: true` resumes delivery for matching future monitor events. It
    also starts sending waiting deliveries at once. Events that happen during
    a pause may never arrive. To catch up, page through
    [List Events](/api-reference/events/list). Use
