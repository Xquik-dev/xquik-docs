...
The Xquik REST API v1 provides programmatic access to monitors, events, webhooks, integrations, automations, draws, extractions, trends, X data lookups, media downloads, tweet styles, drafts, account management, X account connections, and write actions (posting tweets, liking, retweeting, following, DMs, profile updates, media uploads, and community management).
...
<AccordionGroup>
  ...
  <Accordion title="Errors follow a consistent format">
    ...
    | 400 | `invalid_tool_type` | Extraction tool type is not recognized |
    | 400 | `invalid_format` | Export format is not `csv`, `xlsx`, or `md` |
    ...
    | 500 | `internal_error` | Server error - [contact support](mailto:support@xquik.com) if persistent |
    ...
    | 400 | `no_x_account` | No connected X account found for this user |
    | 400 | `x_account_not_found` | Specified X account not connected |
    | 502 | `x_write_failed` | X write action failed - check error message |
