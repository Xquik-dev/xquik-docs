...
9 X-API endpoints also accept anonymous [MPP](/mpp/overview) payments (7 read-only GET endpoints plus POST media/download and GET /trends). The server returns a 402 challenge; your client pays via Tempo (USDC) and retries with a payment credential. No API key needed. See the [MPP overview](/mpp/overview) for eligible endpoints and pricing.
