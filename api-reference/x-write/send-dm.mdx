...
description: "Send a text direct message to an X user from one of your connected accounts"
api: "POST /x/dm/{userId}"
keywords: ["dm", "direct message", "send", "write", "connected account"]
---
...
<ParamField body="media_ids" type="string[]">
  Optional one-item array containing an uploaded media ID. Upload media first with [Upload Media](/api-reference/x-write/upload-media), then pass the returned `mediaId` as the only array item.
</ParamField>
