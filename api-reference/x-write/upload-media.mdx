...
<Tabs>
  <Tab title="200 OK">
    <ResponseField name="mediaId" type="string">The uploaded media ID. Supply it in the `media_ids` array on `POST /x/dm/{userId}` when sending a direct message with media.</ResponseField>
...
<Tip>
  Use the returned `mediaId` in the `media_ids` array when [sending a direct message](/api-reference/x-write/send-dm). Media IDs are valid for 24 hours after upload.
</Tip>

<Note>
  **Related endpoints:** [Send Direct Message](/api-reference/x-write/send-dm) to send a DM with uploaded media, [Create Community](/api-reference/x-write/create-community) to create a new community.
</Note>
