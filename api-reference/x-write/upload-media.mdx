...
<Tip>
  Use the returned `mediaId` as the only item in the `media_ids` array when [sending a direct message](/api-reference/x-write/send-dm). Media IDs are valid for 24 hours after upload.
</Tip>

<Note>
  **Related endpoints:** [Send Direct Message](/api-reference/x-write/send-dm) to send a DM with one uploaded media item, [Create Community](/api-reference/x-write/create-community) to create a new community.
</Note>
