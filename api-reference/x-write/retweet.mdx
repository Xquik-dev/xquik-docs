...
description: "Repost one tweet by ID from a connected X account. Approve the account, send an idempotency key, poll the action, then store billed credits and the result."
...
The rollback needs a new idempotency key. Store both action IDs so the
campaign record shows the repost and its later removal.
