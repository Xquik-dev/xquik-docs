...
description: "Retrieve one monitor event by ID with tweet text, author fields, engagement metrics, event type, timestamps, and monitor identifiers. The call is free."
api: "GET /events/{id}"
...
tweet data, author context, or a support or audit review.
...
| Occurred at         | `occurredAt`                   | Keep the original event timestamp.                 |
...
Event type (for example `tweet.new`, `tweet.reply`, `tweet.quote`, `tweet.retweet`).
