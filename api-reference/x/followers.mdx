...
description: "Get an X account's followers by username or user ID with cursor pagination for CRM, warehouse, audience, and agent workflows. Costs 1 credit per result."
...
Get followers returns follower profiles for one X account by username or numeric user ID. The endpoint is `GET /api/v1/x/users/{id}/followers`.
...
The Node.js and Python snippets write JSON Lines import rows, not raw
follower pages. Store each mapped row and the latest `next_cursor` in your
sync job. It can then resume from the last completed page.
...
job, saved extraction, or CSV, JSON, or XLSX file export.
...
CSV, JSON, or XLSX files after completion.
...
<CardGroup cols={2}>
  ...
  <Card title="Stable upserts" icon="key-round">
    ...
    Store `users[].id` as `x_user_id` for CRM, warehouse, audience, and agent deduplication.

  </Card>
  <Card title="Readable labels" icon="badge">

    Store `users[].username` and `users[].name` for handles, labels, enrichment, and deduplication.

  </Card>
  <Card title="Profile enrichment" icon="file-text">

    Store `users[].description`, `location`, and `url` when returned. The API omits empty profile fields.
...
Paid calls can return fewer rows. `users.length` is the billable row count.
Zero affordable results return
`402 insufficient_credits`.

For high-volume follower pulls, deduplicate profiles by `id`. Continue through empty pages while the cursor advances. Stop with a partial-result status when `next_cursor` is missing or repeats.
...
image beside each ID. Reviewers read those fields. Compare on the numeric ID,
...
| `collected_at`          | Set when you store each cursor page   | Keep collection order and snapshot recency.                   |
...
| `follower_count`        | `users[].followers`                   | Keep the observed audience size for segmentation.             |
| `verified`              | `users[].verified`                    | Keep the observed verification state for filtering.           |
...
| Continuing follower     | Present in both complete snapshots            | Update the username, profile, count, and verification fields. |
| Removed follower        | Present only in the earlier complete snapshot | Add the last-seen time. Do not delete the historical row.     |
| Unknown change          | Either snapshot is partial                    | Keep both snapshots and defer follower-change classification. |
...
pages can leave it out, & `next_cursor` still works.
