...
description: "Retrieve liked tweets that X makes visible for one user, with authors, text, media, and metrics. X shows likes only to their owner. Others get a 424 error."
...
Use this route for liked tweets that X makes visible for one user. X has shown likes only to their owner since 2024. When a profile reports likes and X hides them, the route returns `424 likes_unavailable`. It does not return an empty `200`.
