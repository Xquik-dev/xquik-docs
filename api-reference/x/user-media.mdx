...
engagement counts, `media_urls`, `media_types`, and cursor fields. A worker
can then resume from the last saved `next_cursor`.
...
does not mean you downloaded the asset.
...
| `media_url`            | `tweets[].media[].mediaUrl`      | Deduplicate an asset only within its parent tweet.               |
| `video_variants`       | `tweets[].media[].videoVariants` | Keep every returned rendition until someone approves a download. |
| `page_cursor`          | Request `cursor`                 | Resume the profile media inventory after stored pages.           |
| `collected_at`         | Integration timestamp            | Audit when you observed the media URL and engagement counts.     |
...
same media-tweets target. Xquik filters rows after it fetches each page. Selective
