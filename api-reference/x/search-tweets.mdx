...
`Latest` ranks by time & may add matching posts only `Top` shows. `Top` ranks
engagement. `result_type` and `sort_order` are accepted aliases. `recency` maps
to `Latest`. `relevancy` maps to `Top`.
...
X fails this query on every read, so a retry fails too. Change the query or
its filters. The normalized v1 contract also returns 424 `x_api_unavailable`.
