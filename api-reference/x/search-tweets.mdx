...
description: "Search tweets by keyword, ID, or URL. Return text, authors, replies, metrics, media, and cursors for CRM, agents, or exports. Costs 1 credit per tweet."
...
A `pagination_stalled` response means coverage remains incomplete.
...
Dated author searches keep this exclusion during pagination and recovery.
Use `include:nativeretweets` to include button-based reposts in author searches.
Dated author searches keep this inclusion during pagination and recovery.
`-filter:retweets` also excludes manual RT text. Xquik keeps the requested operator.
Dated author searches with either filter read the account timeline too.
...
<CardGroup cols={2}>
  ...
  <Card title="Relationship context" icon="git-branch">
    ...
    Store `tweets[].media`, `quoted_tweet`, and `retweeted_tweet` to keep attached media and relationship context when available.
...
Tweet search costs 1 credit per tweet returned. Low credit balances can return fewer tweets than `limit`. Zero affordable results return `402 insufficient_credits`. Retry `429` with the `Retry-After` header. Retry `424` or `502` after a short backoff.
