...
Direct replies cost 1 credit per tweet returned. Retry `429` with `Retry-After`. Retry `502` after a short backoff. Retry `503` after its stated delay.
...
Collected rows & diagnostics remain available. `recommendedFallback` says what to do next.
