...
An empty page is a valid search result. It does not mean the community
...
query parameters. Do not advance the cursor until you have saved every row.

`Latest` results change as new tweets arrive. Deduplicate incoming rows by Tweet ID. Keep
earlier decisions when the same tweet appears again.

`Top` results rank by relevance. Record each collection time because ranking
...
Save the exact query before requesting tweets. Keep every operator, quoted
...
Store each cursor page before requesting another. Deduplicate repeated Tweet
