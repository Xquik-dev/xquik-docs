...
This mentions timeline endpoint accepts a username or numeric user ID. It
returns one JSON page at a time. Use [`mention_extractor`](/api-reference/extractions/create)
when you need a saved extraction, estimate, or CSV, JSON, or XLSX file export.
...
| `tweet_id`            | `tweets[].id`               | Deduplicate mentions and open one source tweet.     |
...
same mentions target. Xquik filters rows after it fetches each page. Selective
