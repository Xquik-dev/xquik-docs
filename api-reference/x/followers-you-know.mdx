...
description: "Retrieve mutual X followers between the authenticated context and one target user for warm-intro, CRM, scoring, and agent workflows. 1 credit per result."
...
context and one target X user. The endpoint is
...
print the full response page. Store the rows with the checkpoint so a
...
<CardGroup cols={2}>
  ...
  <Card title="Approved contact" icon="message-square">
    ...
    Use DM endpoints only after a user-approved message flow. The write response tells you whether the DM was delivered.
