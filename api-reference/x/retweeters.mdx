...
initial request. Pass it back unchanged only when `has_next_page` is true. An
edited cursor or one from another tweet returns `400 invalid_cursor`.

</ParamField>

<ParamField query="pageSize" type="integer">

Profiles per page. Range: `1-200`. Defaults to `200`. X can return more
profiles than a page asks for. The next page returns the ones you have not got
yet, so smaller pages skip & repeat no retweeters.
