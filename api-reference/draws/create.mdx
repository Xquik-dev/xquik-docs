...
Full tweet URL to run the draw on. Accepts `x.com` and `twitter.com` formats (for example `https://x.com/user/status/1893456789012345678`). A retweet URL draws from the original post.
...
<ResponseField name="tweetId" type="string">X tweet ID the draw read. For a retweet URL, the original post's ID.</ResponseField>
