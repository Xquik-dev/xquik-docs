...
Remaining credits cap how many replies and retweeters Xquik can inspect before filters run. `totalEntries` and `validEntries` count the inspected replies. The source tweet can have more.
...
The available balance cannot cover the minimum draw cost. A later final deduction failure also returns `insufficient_credits`. Xquik stores no draw result after that failure. Check your [credit balance](/api-reference/account/get).
