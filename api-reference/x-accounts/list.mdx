...
| Value | Meaning |
| --- | --- |
| `healthy` | Cookies are valid. Writes can proceed. |
| `needsReauth` | Credentials were rejected. Submit fresh credentials via [reauth](/api-reference/x-accounts/reauth). |
| `locked` | X locked the account. Unlock at `x.com/account/access`, then reauth. |
| `suspended` | X banned the account. Appeal on X; writes will not recover automatically. |
| `recovering` | Past a transient cooldown. Will auto-retry on next use. |
