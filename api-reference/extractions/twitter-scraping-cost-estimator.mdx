...
description: "Estimate API credits before exporting tweet searches, replies, followers, following, profiles, communities, or lists. The quote is free and starts no job."
...
The `source` field names the count behind the estimate. Examples include `followers`, `following`, `paginationCap`, `replyCount`, `quoteCount`, `retweetCount`, and `resultsLimit`. Save this field beside the credit quote.
...
- `source` names the count behind the estimate.
