...
| `follower.lost` | Existing follower lost |
...
<Tabs>
  ...
  <Tab title="402 Subscription Required">
    ...
    ```
    An active subscription is required to create monitors. Subscribe from the [dashboard](https://xquik.com/subscription).
  </Tab>
  ...
  <Tab title="422 Shadow Account">
    ```json
    { "error": "shadow_account", "message": "This account appears to be shadow-banned and cannot be monitored" }
    ```
    The target account appears to be shadow-banned and cannot be monitored.
  </Tab>
  <Tab title="502 Stream Registration Failed">
    ```json
    { "error": "stream_registration_failed", "message": "Failed to register with streaming service" }
    ```
