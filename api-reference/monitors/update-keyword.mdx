...
description: "Update a keyword monitor's tweet event types or pause and resume it. The X search query stays fixed, so the monitor ID and stored events remain usable."
...
## When to update a keyword monitor

Use this route to change one keyword monitor's event types or active state. Use get to read one monitor. Use list to see every keyword monitor on the account.
...
You cannot change the monitored `query`. Delete this monitor and create another one to track a different query.
...
| Creation time                 | Response `createdAt`              | Keep the original creation timestamp.          |
