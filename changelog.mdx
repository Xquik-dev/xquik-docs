...
- List pages no longer skip rows when X returns more than `pageSize`
- The next page returns the rows you have not got yet, each once
- This covers retweeters & standard followers, following & verified followers
- It also covers other list & timeline pages
- Pass `next_cursor` back unchanged
- An edited cursor or one from another list returns `400 invalid_cursor`
...
- Small conversation searches use automatic pagination recovery by default
...
- Reply recovery keeps collected rows & reports incomplete coverage
...
- Search recovery keeps quote and manual RT distinctions between retweet exclusions
...
- Obsolete public `proxy_country` and `loginCountry` account fields. Xquik does not support custom, dedicated, or fixed per-account proxy configuration.
