...
<AccordionGroup>
  ...
  <Accordion title="Monitor alert routing">
    ...
    Cost: active monitor billing includes webhook management and deliveries.
...
<CardGroup cols={2}>
  ...
  <Card title="4. Monitor tweets to signed webhooks" icon="radio">
    ...
    Cost model: 21 credits per active monitor-hour. The rate includes webhook delivery.
...
For query alerts, call `POST /monitors/keywords` with `query` and `eventTypes` instead of `username`. Account and keyword monitors both check every 1 second while active. Both can deliver events to the same signed webhooks.

Each POST has 1 event. Multiple matches produce multiple POSTs.

Payloads include durable IDs. Store `deliveryId` for delivery-level idempotency. Store `streamEventId` when you must process the same monitor event once across retries or endpoints. Store the one-time `secret` returned by `POST /webhooks`. Verify `X-Xquik-Signature` against the raw body before accepting the event. Return `2xx` before slow CRM, Slack, warehouse, or queue work starts.
...
History reads require `account`. That connected account must participate in the conversation. Missing `account` returns `400 account_required`. A non-participant account returns `403 dm_not_permitted`.
...
`GET /x/dm/{userId}/history` returns `messages`, `has_next_page`, and `next_cursor`. Pass `next_cursor` back as `cursor` for older messages. `POST /x/dm/{userId}` returns `messageId` and `success`. Store both on the support ticket, CRM note, or agent state.
...
<CardGroup cols={2}>
  ...
  <Card title="Follower CRM export" icon="users" href="/guides/follower-export-crm">
    ...
    Estimate `follower_explorer`, export CSV, JSON, or XLSX files, and map stable `x_user_id` fields.
...
Completed giveaway draws have a public results page at `https://xquik.com/results/{drawId}`. The page needs no authentication. Share the URL with participants for transparency.
