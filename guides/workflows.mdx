...
Compose algorithm-optimized tweets using a 3-step workflow: get algorithm knowledge, refine with preferences, then score the draft. All 3 tools are free and do not consume usage quota.

### Step 1: Start Composing

Get X algorithm engagement signals and follow-up questions.

<CodeGroup>

```bash cURL
curl -X POST https://xquik.com/api/v1/x/compose-tweet \
  -H "x-api-key: xq_YOUR_KEY_HERE" \
  -H "Content-Type: application/json" \
  -d '{
...
```javascript Node.js
const response = await fetch(`${BASE}/x/compose-tweet`, {
  method: "POST",
  headers: {
    "x-api-key": API_KEY,
    "Content-Type": "application/json",
...
const { signals, rules, penalties, followUpQuestions } = await response.json();
...
print("Follow-up questions:", data["followUpQuestions"])
```

```go Go
func composeTweet(topic, goal string) map[string]interface{} {
	payload, _ := json.Marshal(map[string]string{
		"topic": topic,
		"goal":  goal,
	})

	req, _ := http.NewRequest("POST", base+"/x/compose-tweet", bytes.NewReader(payload))
	req.Header.Set("x-api-key", apiKey)
	req.Header.Set("Content-Type", "application/json")
	...
	json.Unmarshal(body, &result)
	fmt.Println("Signals:", result["signals"])
	return result
}
```
...
Pass the user's answers to get targeted composition guidance.

<CodeGroup>

```bash cURL
curl -X POST https://xquik.com/api/v1/x/refine-tweet \
  -H "x-api-key: xq_YOUR_KEY_HERE" \
  -H "Content-Type: application/json" \
  -d '{
    "topic": "launching my new SaaS product",
...
```javascript Node.js
const refineResponse = await fetch(`${BASE}/x/refine-tweet`, {
  method: "POST",
  headers: {
    "x-api-key": API_KEY,
    "Content-Type": "application/json",
...
const { guidance, examplePatterns } = await refineResponse.json();
console.log("Guidance:", guidance);
console.log("Example patterns:", examplePatterns);
```

```python Python
response = requests.post(
    f"{BASE}/x/refine-tweet",
    headers={"x-api-key": API_KEY},
    json={
        "topic": "launching my new SaaS product",
        "goal": "engagement",
...
print("Example patterns:", data["examplePatterns"])
```

```go Go
func refineTweet() map[string]interface{} {
	payload, _ := json.Marshal(map[string]string{
		"topic":        "launching my new SaaS product",
		"goal":         "engagement",
		"tone":         "casual",
	...
	req, _ := http.NewRequest("POST", base+"/x/refine-tweet", bytes.NewReader(payload))
	req.Header.Set("x-api-key", apiKey)
	req.Header.Set("Content-Type", "application/json")
	...
	var result map[string]interface{}
	json.Unmarshal(body, &result)
	fmt.Println("Guidance:", result["guidance"])
	return result
}
```
...
Evaluate the draft tweet against 8 algorithm ranking factors.

<CodeGroup>

```bash cURL
curl -X POST https://xquik.com/api/v1/x/score-tweet \
  -H "x-api-key: xq_YOUR_KEY_HERE" \
  -H "Content-Type: application/json" \
  -d '{
...
```javascript Node.js
const scoreResponse = await fetch(`${BASE}/x/score-tweet`, {
  method: "POST",
  headers: {
    "x-api-key": API_KEY,
    "Content-Type": "application/json",
...
const { totalChecks, passedCount, topSuggestion, checklist } =
  await scoreResponse.json();
console.log(`Score: ${passedCount}/${totalChecks}`);
if (topSuggestion) console.log("Top suggestion:", topSuggestion);
```

```python Python
response = requests.post(
    f"{BASE}/x/score-tweet",
    headers={"x-api-key": API_KEY},
    json={
        "draft": "Just shipped dark mode for our app.\n\nWhat do you think?",
        "hasMedia": True,
...
data = response.json()
print(f"Score: {data['passedCount']}/{data['totalChecks']}")
if data.get("topSuggestion"):
    print("Top suggestion:", data["topSuggestion"])
```

```go Go
func scoreTweet(draft string, hasMedia bool) {
	payload, _ := json.Marshal(map[string]interface{}{
		"draft":    draft,
		"hasMedia": hasMedia,
	})

	req, _ := http.NewRequest("POST", base+"/x/score-tweet", bytes.NewReader(payload))
	req.Header.Set("x-api-key", apiKey)
	req.Header.Set("Content-Type", "application/json")
