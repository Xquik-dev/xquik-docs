...
keywords: ["integration patterns", "monitor and poll", "webhooks workflow", "giveaway automation", "tweet composition", "end-to-end"]
---
...
The agent calls the `monitors` tool:
...
The agent calls the `events` tool:
...
<Info>
  The MCP `events` tool uses different field names than the REST API (`eventType` vs `type`, `eventData` vs `data`, `xUsername` vs `username`, `monitoredAccountId` vs `monitorId`). Both return the same underlying data.
</Info>
...
Compose algorithm-optimized tweets using a 3-step MCP workflow: get algorithm knowledge, refine with preferences, then score the draft. All 3 tools are free and do not consume usage quota.

<Info>
  Tweet composition is available exclusively through the [MCP server](/mcp/overview). There are no REST API endpoints for these tools. Connect your AI agent via MCP and use natural language to compose tweets.
</Info>
...
The agent calls the `compose-tweet` tool:
...
After you answer the follow-up questions, the agent calls the `compose-tweet` tool with `step="refine"`:
...
```json
{
  "guidance": "Lead with a personal story or metric. End with an open question. Attach a product screenshot.",
  "examplePatterns": [
    "Hook: surprising metric or confession → Context: 1-2 sentences → CTA: open question",
    "Hook: bold claim → Evidence: screenshot or demo → CTA: 'What would you build?'"
  ]
}
```
...
Once the agent drafts a tweet, it evaluates the text by calling the `compose-tweet` tool with `step="score"`:
...
<Note>
  All 3 tweet composition steps are free and do not count toward your monthly usage quota. Iterate on your draft and re-score until all checks pass.
</Note>
...
<CardGroup cols={2}>
  ...
  <Card title="MCP Tools" icon="wrench" href="/mcp/tools">
    Full input/output schemas for all 18 tools.
  </Card>
</CardGroup>
