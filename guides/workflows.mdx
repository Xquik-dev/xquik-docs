...
<Info>
  The MCP `get-events` tool uses different field names than the REST API (`eventType` vs `type`, `eventData` vs `data`, `xUsername` vs `username`, `monitoredAccountId` vs `monitorId`). Both return the same underlying data.
</Info>
...
After you answer the follow-up questions, the agent calls the `refine-tweet` tool:
...
Once the agent drafts a tweet, it evaluates the text by calling the `score-tweet` tool:
...
<Note>
  All 3 tweet composition tools are free and do not count toward your monthly usage quota. Iterate on your draft and re-score until all checks pass.
</Note>
...
<CardGroup cols={2}>
  ...
  <Card title="MCP Tools" icon="wrench" href="/mcp/tools">
    Full input/output schemas for all 39 tools.
  </Card>
</CardGroup>
