...
| Tier | Price | Included credits | Monitor allowance | Best for |
|------|-------|------------------|-------------------|----------|
| Starter | USD 20/month | 140,000 credits (USD 0.00014/credit) | 1 monitor | Prototyping and low-volume integrations |
| Pro | USD 99/month | 770,000 credits (USD 0.00013/credit) | 1 monitor | Production workloads and growing teams |
| Business | USD 199/month | 1,670,000 credits (USD 0.00012/credit) | 1 monitor | High-volume automation and enterprise use |

Higher tiers give you more credits at a lower per-credit cost. PAYG credits cost USD 0.00015 each.

All tiers include 1 monitor. Additional monitors are USD 5/month each. Payment via credit/debit card.
...
- Managing monitors, reading stored monitor events, webhooks, and account endpoints are free. Capturing new monitor events is metered.
...
| Monitor event capture | per captured event | 1 |
...
The following do not consume any credits: managing monitors (create, get, update, delete), reading stored monitor events, and all webhook operations (create, get, update, delete, deliveries, test); reading, fetching, and exporting extractions and draws; cost estimates; tweet composition (compose, refine, score); style cache management; drafts; support tickets; API key management; X account management; and credit top-ups ([checkout](/api-reference/credits/topup) and [quick top-up](/api-reference/credits/quick-topup)). Account status and [radar](/api-reference/radar/list) are also free. Capturing new monitor events costs 1 credit per event, and a monitor pauses when an event cannot be charged.
...
| `GET /x/articles/{id}` | USD 0.00105 per call | charge |
| `POST /x/media/download` | USD 0.00015 per media item | session |
...
Metered calls return `402 Payment Required`. Managing monitors, reading stored monitor events, webhooks, and account endpoints continue working. New monitor event capture pauses when an event cannot be charged. [Top up credits](/api-reference/credits/topup) from the dashboard to continue.
