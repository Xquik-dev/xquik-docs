...
Extractions and draws consume quota proportional to the number of results processed. Use the [estimate endpoint](/api-reference/extractions/estimate) or the `xquik` MCP tool with `xquik.request('/api/v1/extractions/estimate', { method: 'POST', body: {...} })` to preview costs before running large jobs.
