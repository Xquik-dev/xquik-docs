...
Every subscription includes a generous monthly usage quota that resets each billing period. The quota covers operations that query the X API (searches, lookups, fetches). Management operations (monitors, webhooks, account) are unlimited.
...
Compare `quotaUsed` against `quotaLimit` to calculate remaining capacity. Both values are opaque bigint strings - compare them numerically, not as dollar amounts.

<Tip>
  Poll the account endpoint periodically to track usage. Build alerts when `quotaUsed` exceeds 80% of `quotaLimit` to avoid hitting the cap unexpectedly.
</Tip>
