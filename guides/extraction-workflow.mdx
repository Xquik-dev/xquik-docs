...
Every result includes `id`, `xUserId`, and `createdAt`. The API omits all other fields when unavailable (never `null`). Check for field presence before accessing.
...
Xquik caps exports at 100,000 rows (10,000 for PDF). For larger extractions, use the paginated API to retrieve all results.
