...
<CardGroup cols={2}>
  ...
  <Card title="Liked tweets" icon="heart">
    ...
    Use `user_likes` with `targetUsername` to extract liked tweets that X makes visible. X shows likes only to their owner. The job reads through your connected X account. Without one, the job fails.
