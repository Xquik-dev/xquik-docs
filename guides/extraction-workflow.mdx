...
| `POST /extractions/estimate` | `extractions` with `action="estimate"` | Preview cost |
| `POST /extractions` | `extractions` with `action="run"` | Start the job |
| `GET /extractions/:id` | `extractions` with `action="get"` | Retrieve results |
...
<Note>
  File export (`GET /extractions/:id/export`) is only available via the REST API. The MCP server returns results as structured JSON through the `extractions` tool with `action="get"`.
</Note>
