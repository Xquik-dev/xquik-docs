...
console.log(`Job ${extraction.id}: ${extraction.status} (${extraction.totalResults} results)`);
```
...
print(f"Job {extraction['id']}: {extraction['status']} ({extraction['totalResults']} results)")
```
...
fmt.Printf("Job %s: %s (%v results)\n", extraction["id"], extraction["status"], extraction["totalResults"])
```
</CodeGroup>
