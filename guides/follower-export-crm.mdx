...
<CardGroup cols={2}>
  ...
  <Card title="4. Export" icon="download">
    ...
    Exports are free. Xquik caps them at 100,000 rows per extraction. PDF supports up
    to 10,000 rows.
...
<CardGroup cols={2}>
  ...
  <Card title="Saved follower export" icon="archive">
    ...
    Use `POST /extractions/estimate`, then `POST /extractions` with `follower_explorer` when you need a reusable job ID, cost preview, and CSV, JSON, or XLSX files.
