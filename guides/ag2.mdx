...
`api_key` is required. The toolkit raises `ValueError` when it is empty, so a missing key fails at construction, not on the first search.
...
The toolkit passes tweet records through unmodified, so the agent receives every tweet ID, profile ID, and timestamp. Feed `next_cursor` back through the `cursor` parameter to continue pagination.
