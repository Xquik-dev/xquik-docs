...
[AG2](https://github.com/ag2ai/ag2) is an open-source Python framework for multi-agent systems. AG2 1.0.0 and later ship `XquikSearchToolkit` in `ag2.extensions.tools.search`. Xquik tweet search reaches AG2 agents without a community adapter. AG2 documents the toolkit in its [Xquik Tweet Search reference](https://docs.ag2.ai/docs/user-guide/extensions/tools/search/xquik/). Keep every tweet ID and cursor the API returns.
...
Use AG2 for research, monitoring, and reporting agents. Use direct REST for deterministic jobs that need no model decisions.
...
`XquikSearchToolkit` calls the Xquik REST API over `httpx`, which AG2 already depends on. You need no extra package. Install your model provider extra as well.
...
`api_key` is required. The toolkit raises `ValueError` when it is empty. A missing key fails at construction, not on the first search.

## Bind the search window outside model control

Set search defaults on the constructor. The model then cannot widen the window or raise the result limit.
...
The `search()` factory method takes the same parameters when you want several differently scoped tools from one toolkit.
...
`xquik_tweet_search` returns a typed response, not a raw payload.

| Field           | Type         | Meaning                                     |
| --------------- | ------------ | ------------------------------------------- |
| `query`         | `str`        | The searched query                          |
| `tweets`        | `list[dict]` | Tweet records as the API returned them      |
| `has_next_page` | `bool`       | Whether another page exists                 |
| `next_cursor`   | `str`        | Cursor for the next page, empty when absent |

The toolkit passes tweet records through unmodified. The agent receives every tweet ID, profile ID, and timestamp. Feed `next_cursor` back through the `cursor` parameter to continue pagination.

## Resolve values at runtime with variables

Every runtime parameter accepts an AG2 `Variable`. AG2 resolves it from the run context when the tool executes. One tool instance serves many users or tenants.
