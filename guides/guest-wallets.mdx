...
| Guest wallet    | Not required | `paid_reads` API key           | Exactly 33 prepaid GET routes              | USD 10 to 250 hosted checkout                         |
...
`/favoriters`, `/likes` & `/followers-you-know` read through your own connected X account. Guest keys have none, so these 3 routes return `424 account_required`.
