...
Returned by `GET /api/v1/account`. The `currentPeriod` field is omitted when there is no active subscription. `quotaLimit` and `quotaUsed` are opaque bigint strings - compare them numerically to track remaining capacity. Do not parse them as JavaScript numbers.
