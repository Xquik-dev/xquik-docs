...
// Fields vary by event type. Common fields shown below.
...
// follower.gained and follower.lost events
...
interface AutomationStep {
  id: string;
  stepType: "action" | "condition" | "extraction";
...
`Automation` represents a flow automation with a trigger and one or more steps. `AutomationDetail` extends the base type with the full `steps` array and `recentRuns` history. Each `AutomationStep` has a `stepType` that determines whether `actionType` or `extractionType` is populated. `AutomationRun` tracks execution status, timing, and any error messages. The `triggeredBy` field indicates what initiated the run (e.g., a monitor event or manual trigger).
