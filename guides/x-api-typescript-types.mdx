...
Two shapes exist. Only `POST /api/v1/api-keys` returns `ApiKeyCreated`, which includes the `fullKey` field. This is the only time the API exposes the full key. `GET /api/v1/api-keys` returns `ApiKey`, which shows only the `prefix` (first 8 characters) for identification.
