...
<AccordionGroup>
  ...
  <Accordion title="Write action">
    An operation that modifies X state — tweet, like, follow, DM, etc. Write actions cost 3-7 credits and require a connected X account.
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion title="Fixed window counter">
    The rate-limiting algorithm used by Xquik. Each API key gets a counter per tier (read, write, delete) that resets every 60 seconds. Requests are rejected with 429 when the counter exceeds the tier limit. See [rate limits](/guides/rate-limits) for details.
  </Accordion>
</AccordionGroup>
