...
<AccordionGroup>
  <Accordion title="Monitor">
    A persistent watcher on an X user that streams events via webhooks. Monitors run continuously and detect changes like new tweets, follower gains, and profile updates. Each subscription includes 1 monitor; additional monitors are USD 5/month each.
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="Non-metered">
    Operations that are free and do not consume credits. Includes monitor management, stored event reads, webhooks, and account management endpoints. Capturing new monitor events is metered.
  </Accordion>
