...
<AccordionGroup>
  ...
  <Accordion title="Token bucket">
    The rate-limiting algorithm used by Xquik. Tokens refill at a fixed rate per second; each request consumes one token. Bursts are allowed up to the bucket capacity. See [rate limits](/guides/rate-limits) for details.
  </Accordion>
</AccordionGroup>
