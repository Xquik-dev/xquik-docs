...
| Component | Role |
|-----------|------|
| **REST API** | 98 endpoints at `https://xquik.com/api/v1/*` for programmatic access |
...
Rate limits are enforced per API key using a **fixed-window counter** algorithm. Each tier has an independent counter that resets every 60 seconds.
...
When the limit is reached, requests return `429 Too Many Requests` with a `Retry-After: 60` header.
