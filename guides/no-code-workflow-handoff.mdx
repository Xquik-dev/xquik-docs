...
This no code API integration pattern keeps cursors and event IDs. Choose
...
| Outcome                               | Best path               | Required checkpoint     |
| ------------------------------------- | ----------------------- | ----------------------- |
...
Choose the path from the required freshness and row count. Do not poll a
...
<CardGroup cols={2}>
  ...
  <Card title="n8n" icon="workflow" href="/guides/n8n">
    ...
    cursor, and deduplication checkpoints. Use persistent state in production.
...
Webhook deliveries retry with backoff until your endpoint returns `2xx`. There
is no attempt limit. Return `2xx` for events your workflow skips. Webhook
health exposes `deliveryStatus`, `consecutiveFailures`, and `failureHardCap`.
`needs_attention` does not stop retries. Only a paused or deleted webhook marks
...
4. Fix the receiver.
5. Call the resume endpoint. It starts sending waiting and rejected deliveries
   at once.
6. Replay only event IDs your workflow never stored.

A paused webhook keeps waiting deliveries for 30 days after each event.
...
<AccordionGroup>
  ...
  <Accordion title="402 payment required">
    ...
    Credits required. Add credits or subscribe, then resume from the checkpoint.
  ...
  <Accordion title="429 rate limited">
    ...
    Rate limited. Respect the retry delay and keep the current cursor.
...
Use an Xquik REST Hook for monitor events. Use API request actions for
...
Request nodes. Receive monitor events through a Webhook node. Store cursors
...
Use the no-code platform's scheduler to schedule tweets. Use Xquik only for
...
They are safe with vaulted credentials, approved writes, and capped retries. Keep API
