...
| Agent task            | Xquik route                      | Keep for the next step                            |
| --------------------- | -------------------------------- | ------------------------------------------------- |
...
| Watch keywords        | `POST /api/v1/monitors/keywords` | Monitor ID, event types, and next billing time    |
...
The `execute` tool runs a bounded sandbox function. That function calls
...
## Keep the MCP response contract

MCP returns normalized snake_case fields and Unix-second timestamps. A REST
`createdAt` field becomes `created`, not `created_at`. Keep source values
...
advances. Deduplicate tweets and users by their stable `id` values.
...
Conversation history is not a reliable job database. Store the values needed
...
| `424`  | Upstream dependency failure or incomplete replies | Keep partial rows and inspect `error.retryable`  |
...
Keep exact error messages for operators. Record where the error occurred.
...
Xquik exposes reads and writes through one aggregate `execute` tool.
...
Save graph state after each external call. Store the last completed node,
...
Yes. Call `GET /api/v1/x/tweets/search` through MCP. Keep each tweet ID,
...
Yes. Connect the target X account first. Require human approval, keep the
idempotency key, and store the returned tweet or write-action ID.

### How do I authenticate with the Twitter API using Python?

Send an Xquik API key in the MCP `x-api-key` header. You need no X Developer
keys. Some private reads and writes require a connected Twitter account.
...
Validate a Pydantic schema first. Store typed tweet IDs, fields, cursors,
...
Monitor events are asynchronous. Xquik does not guarantee real-time delivery.
