...
description: "Test signed tweet, profile, and keyword monitor webhook deliveries with a local server, ngrok, or webhook.site. Verify HMAC signatures. Follow exact steps."
keywords:
  [
    "webhook testing",
    "signed webhook",
...
They omit `deliveryId` and `streamEventId`. Use them for reachability and
...
<CardGroup cols={2}>
  ...
  <Card title="Production deduplication" icon="copy-check">
    ...
    Use `deliveryId` for receiver retry deduplication. Use `streamEventId` when one
...
rows carry the IDs your receiver needs for retry deduplication, event deduplication, and
event detail joins.
...
the raw body available for the HMAC check, then store only the deduplication IDs,
...
Join `streamEventId` to [Get Event](/api-reference/events/get) to find the
monitor event behind a failed delivery.
...
history. The delivery `streamEventId` is the event `id`. A failed receiver
...
<CardGroup cols={2}>
  ...
  <Card title="pending" icon="clock">
    ...
    Not sent yet. It may wait while your endpoint is failing. Check recent
    deploys, tunnel uptime, and receiver availability.
  ...
  <Card title="failed" icon="triangle-alert">
    ...
    The latest attempt failed. Xquik retries with backoff until your endpoint
    returns `2xx`. Inspect `lastStatusCode`, `lastError`, and your receiver
    logs.

  </Card>
  <Card title="exhausted" icon="pause">

    The webhook was paused or deleted while this delivery waited. Resume it
    within 30 days of the event to receive the delivery.
