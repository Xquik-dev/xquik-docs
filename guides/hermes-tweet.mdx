...
The read tool keeps complete JSON payloads. It does not drop tweet, profile, media, or pagination fields.
...
Keep `next_cursor` whenever `has_next_page` is true. Continue only when the workflow needs another page.
...
A Twitter follower export supports audience analysis and CRM enrichment. A following export lists accounts that the target follows.
...
Store `deliveryId` and `streamEventId` for deduplication. Return `2xx` for duplicates you already accepted.
...
<CardGroup cols={2}>
  ...
  <Card title="Monitor webhook action" icon="radio">
    ...
    `2xx` for accepted duplicates. Keep endpoint signing values, raw
...
For scriptable checks, use `hermes tools list`. Bare `hermes tools` opens the interactive tool UI and requires a TTY. The list command reports plugin toolsets. Confirm `hermes-tweet` appears before testing calls.

```bash
hermes tools list
```

Run a non-mutating one-shot probe after you enable the plugin:
...
Store tweet IDs, author IDs, timestamps, reply text, and available metrics. Keep cursors or job status for reproducible collection.
...
No. It needs no X developer credentials. Hermes Tweet authenticates with an Xquik API key.
