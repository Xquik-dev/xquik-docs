...
The current package version is `0.1.0`. The plugin name is `hermes-tweet`, and the Python entry point is `hermes-tweet = hermes_tweet`.
...
| Symptom | Fix |
|---------|-----|
| Plugin appears but tools are missing | Run `hermes plugins enable hermes-tweet` and restart Hermes |
| Auth fails | Confirm `XQUIK_API_KEY` is exported in the same environment that starts Hermes |
