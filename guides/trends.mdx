...
If you use Xquik via the [MCP server](/mcp/overview), ask your AI agent directly:

- "What's trending in the US?"
- "Show me worldwide trends"
- "Search tweets about the top trend"

The MCP server exposes a `list-trends` tool that maps to `GET /api/v1/trends`. AI agents can chain it with `search-tweets` to explore trending conversations automatically.
