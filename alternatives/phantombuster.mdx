...
PhantomBuster's official API help describes launching an individual Phantom through `POST /agents/launch` with an Agent ID. It says the run uses the Phantom's saved configuration by default, optional launch arguments can override that setup, and arrays are not supported. It also says successful launches return status `200` with a Container ID, and Workflows cannot be launched through the API.
...
| API fit        | Official API docs cover launching Phantoms and managing containers, but state that the API cannot launch Workflows.                                                            | REST endpoints cover tweet search results, follower exports, account actions & monitor events, X write actions, monitors, webhooks, extractions, billing, accounts, support, and API keys. |
