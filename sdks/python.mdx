...
Use this workflow when a Python script, notebook, worker, or agent tool needs every reply under one tweet as a saved extraction, JSON Lines handoff, or CSV, JSON, or XLSX file export. It uses `client.extractions.estimate_cost`, `run`, `retrieve`, and `export_results`.
...
Use `with_raw_response.create` to capture the durable write action. Send a unique `Idempotency-Key`. Store `id`, `request.hash`, `billing`, `result`, and `statusUrl`. Poll while `terminal` is false. Retry only when `safeToRetry` is true, using a new key.
