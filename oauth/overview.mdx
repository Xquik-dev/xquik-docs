...
Authorization errors are returned as HTML pages (standard OAuth browser-facing behavior), not JSON responses.
...
| `invalid_target` | Resource is not `https://xquik.com/mcp` |
| `slow_down` | Rate limited (too many token requests) |
| `unauthorized_client` | Client not authorized for the requested grant type |
...
| `invalid_client_metadata` | Unsupported grant types or `client_name` exceeding 256 characters |
| `Too many redirect URIs` | Exceeds the 10 redirect URI limit |
| `slow_down` | Rate limited |

**Registration limits:**
- Maximum **10** redirect URIs per client
- Maximum **256** character client name
