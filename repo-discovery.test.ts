...
const REQUIRED_N8N_GUIDE_SNIPPETS = [
  ...
  "For storage rows, map production `deliveryId` to `delivery_id` for receiver retry deduplication and `streamEventId` to `stream_event_id` when one monitor event should process once across endpoint changes.",
  '<Card title="Webhook event deduplication" icon="fingerprint">',
  "Store `deliveryId` for receiver retry deduplication and `streamEventId` when one monitor event should process once across endpoint changes.",
  '<Card title="Stored event replay" icon="activity">',
