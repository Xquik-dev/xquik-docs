...
const REQUIRED_SDK_OVERVIEW_SNIPPETS = [
  ...
  "Xquik reserves credits after the job starts",
...
const REQUIRED_CLI_SDK_WORKFLOW_SNIPPETS = [
  ...
  "Store `follower-run.json`, `job_id`, the source username, and the estimate before polling",
  "Credit reservation happens after the job starts.",
  "fetch only the affordable count before export",
  "mark the job `failed` with `insufficient_credits`",
  "followers-page.json",
...
const REQUIRED_MCP_CONTRACT_SNIPPETS = [
  ...
  "Poll `GET /api/v1/extractions/{id}`, export CSV, JSON, or XLSX with `GET /api/v1/extractions/{id}/export`, and store reply rows plus `has_more` and `next_cursor`.",
  "Cost: 1 credit per reply extracted or returned.",
  ...
  "Deduplicate production payloads with `deliveryId` and `streamEventId`. Inspect `GET /api/v1/webhooks/{id}/deliveries` for retry status rows.",
...
const REQUIRED_BILLING_RECOVERY_SNIPPETS = [
  ...
  "get the user's confirmation before billing",
...
const REQUIRED_BILLING_MONITOR_SNIPPETS = [
  ...
  "Hourly cost: 21 credits/hour. Daily estimate: 500 credits/day.",
  '<Card title="5 active monitors" icon="activity">',
  "Hourly cost: 105 credits/hour. Daily estimate: 2,500 credits/day.",
  '<Card title="10 active monitors" icon="gauge">',
  "Hourly cost: 210 credits/hour. Daily estimate: 5,000 credits/day.",
...
const REQUIRED_BILLING_MPP_SNIPPETS = [
  ...
  "Guest wallets prepay the 30 eligible GET routes without an account.",
...
const REQUIRED_GUEST_WALLET_GUIDE_SNIPPETS = [
  "Guest wallets provide prepaid access to 30 eligible X read routes without an account",
  ...
  "The `paid_reads` scope permits exactly the 30 GET routes",
  ...
  "The other 23 routes require a guest or full account credential.",
...
const REQUIRED_LIKE_TWEET_API_SNIPPETS = [
  ...
  "Each request likes 1 tweet by ID.",
  "Each request uses 1 connected X account.",
...
const REQUIRED_TWEET_REPLIES_EXPORT_SNIPPETS = [
  ...
  "Keep author IDs separately from usernames because usernames can change.",
  "## Related reply APIs",
...
const REQUIRED_GET_USER_API_HANDOFF_SNIPPETS = [
  ...
  "Join on the numeric user ID, which does not change.",
...
const REQUIRED_FOLLOWERS_API_HANDOFF_SNIPPETS = [
  ...
  "The endpoint is `GET /api/v1/x/users/{id}/followers`",
  ...
  "write JSON Lines import rows, not raw",
  "Automatic `pageSize` accepts 20 to 300.",
  ...
  "`users.length` is the billable row count.",
...
const REQUIRED_FOLLOWING_API_HANDOFF_SNIPPETS = [
  ...
  "The endpoint accepts a username or numeric user ID",
  "`following_explorer`",
  ...
  '"page_cursor": page_cursor',
  '"next_cursor": page["next_cursor"]',
  "write JSON Lines audience rows, not raw",
  "Pass `next_cursor` back unchanged.",
  "Existing unprefixed cursors keep legacy behavior.",
...
const REQUIRED_LIST_TWEETS_API_HANDOFF_SNIPPETS = [
  ...
  "You cannot see private Lists that other",
  ...
  '"media_urls": [',
  'next_cursor = data["next_cursor"] if data["has_next_page"] else None',
  "build one row per returned list tweet",
...
const REQUIRED_BATCH_USERS_API_HANDOFF_SNIPPETS = [
  ...
  "Retry `failed_ids` now. Retry `unprocessed_ids` after adding credits.",
  'retry_now = data["failed_ids"]',
  'retry_after_credits = data["unprocessed_ids"]',
  "profile_rows = [",
  "Skip `unavailable_ids`, which have no profile on X.",
  "snippets build profile rows. They do not print",
  "`requested_ids`, `user_id`, `username`,",
  "`has_next_page`, and `next_cursor`",
  "Join returned users by `user_id`. Do not rely on response order.",
  "100 IDs per request",
  "`has_next_page: false`",
  '`next_cursor: ""`',
  "`402 insufficient_credits`",
  ...
  "[`GET /x/tweets?ids=`](/api-reference/x/batch-tweets)",
...
const REQUIRED_USER_MENTIONS_API_HANDOFF_SNIPPETS = [
  ...
  "This mentions timeline endpoint accepts a username or numeric user ID",
...
const REQUIRED_VERIFIED_FOLLOWERS_API_HANDOFF_SNIPPETS = [
  ...
  "snippets build verified follower rows. They do not",
...
const REQUIRED_TWEET_SEARCH_EXPORT_SNIPPETS = [
  ...
  "A bare q=from:username request uses automatic timeline and search coverage.",
  "Continue while the response includes next_cursor.",
  "Use mode=standard with limit only when an old integration needs the legacy timeline read.",
...
const REQUIRED_USER_TWEETS_API_HANDOFF_SNIPPETS = [
  ...
  "This endpoint accepts a username or numeric",
...
const REQUIRED_EXTRACTION_WORKFLOW_SNIPPETS = [
  ...
  "Use this workflow to scrape tweets, export followers, pull tweet replies, save CSV, JSON, or XLSX files, or hand paginated JSON to a CRM, warehouse, queue, or AI agent.",
  ...
  "fail with `insufficient_credits`",
  "You need no active plan when enough",
  ...
  "The export limit does not cap paginated JSON.",
  "Xquik caps file exports at 100,000 rows and PDF exports at 10,000 rows.",
...
const REQUIRED_RESPONSE_FORMATS_EXPORTS_SNIPPETS = [
  ...
  "Xquik caps file exports at 100,000 rows and PDF",
  "exports at 10,000 rows.",
...
const REQUIRED_MEDIA_UPLOAD_WORKFLOW_SNIPPETS = [
  ...
  "If you already have public image URLs or a public MP4 video URL for a tweet or reply, skip `POST /x/media`. Pass those URLs in the `media` array on `POST /x/tweets`.",
...
const REQUIRED_UPLOAD_MEDIA_API_HANDOFF_SNIPPETS = [
  ...
  "`POST /x/media`",
  "The route accepts one local file",
  "Each request uses one connected",
  ...
  "Match the content",
  "type to the file.",
  "Match the content type to the file.",
  "Use a public URL.",
  "Xquik rejects private and reserved addresses.",
...
const REQUIRED_DIRECT_MESSAGE_WORKFLOW_SNIPPETS = [
  ...
  "Use your own job timestamp if the next system needs `sent_at`",
...
const REQUIRED_WEBHOOK_TESTING_SNIPPETS = [
  ...
  "signature checks, not production deduplication.",
  '"success": true',
  '"statusCode": 200',
  ...
  '<Card title="Production deduplication" icon="copy-check">',
  ...
  "store only the deduplication IDs,",
...
const REQUIRED_WEBHOOK_RESUME_API_SNIPPETS = [
  ...
  "resets `consecutiveFailures` to `0` and returns the updated webhook object. Xquik then starts sending waiting deliveries at once.",
  "Events that happen during a pause may never arrive. To catch up, page through [List Events](/api-reference/events/list).",
  "If the signed test fails, Xquik returns `400` with `success: false`, `statusCode`, and `error`. Xquik does not reactivate the webhook.",
...
const REQUIRED_WEBHOOK_UPDATE_API_SNIPPETS = [
  ...
  "These snippets build a reconfiguration row.",
  ...
  "new posts that are not replies, quotes, or retweets.",
  '<Card title="tweet.quote" icon="quote">',
  "your systems handle quote payloads.",
  '<Card title="tweet.reply" icon="message-circle">',
  "reply alerts or support routing should continue.",
  '<Card title="tweet.retweet" icon="repeat-2">',
  "repost activity should keep triggering deliveries.",
  "Only the [Test Webhook](/api-reference/webhooks/test) endpoint generates `webhook.test`.",
  ...
  "`isActive: true` resumes delivery for matching future monitor events. It also starts sending waiting deliveries at once. Events that happen during a pause may never arrive.",
  "[Resume Webhook](/api-reference/webhooks/resume)",
  "This endpoint does not rotate or return `secret`.",
  '<Card title="Delivery check" icon="activity">',
  "[List Deliveries](/api-reference/webhooks/deliveries)",
  "`status`, `attempts`, `lastStatusCode`, `lastError`, `createdAt`, and `deliveredAt`",
  '<Card title="Event join" icon="link">',
  "Use delivery `streamEventId` as the `{id}` for [Get Event](/api-reference/events/get). Store `monitorId`, `monitorType`, `type`, `occurredAt`, and `data`",
  "[Create Webhook](/api-reference/webhooks/create)",
] as const;
...
const REQUIRED_WEBHOOK_DELETE_API_SNIPPETS = [
  ...
  '<Card title="Delivery audit" icon="activity">',
  "Delivery rows for inactive endpoints can show `pending`, `failed`, or `exhausted`. Reactivate within 30 days of their event to receive them. Store `streamEventId`, `status`, `attempts`, `lastStatusCode`, `lastError`, `createdAt`, and `deliveredAt`",
  '<Card title="Event join" icon="link">',
  "Use delivery `streamEventId` as the `{id}`",
  "[Get Event](/api-reference/events/get)",
  "Store `monitorId`, `monitorType`,",
...
const REQUIRED_WEBHOOK_DELIVERIES_API_SNIPPETS = [
  ...
  "action:",
  "const failedDeliveries = deliveryTriage.filter",
  "delivery_triage = [",
  ...
  "Use `attempts` to see how often Xquik tried a delivery.",
  ...
  '"attempts": 12',
  "Xquik retries failed deliveries with no attempt limit. While your endpoint fails, Xquik retries 1 waiting delivery at a time. It checks the endpoint at least every 15 minutes. New deliveries wait as `pending`. Return `2xx` for a delivery you will never accept. See the [retry policy](/webhooks/overview#retry-policy). Fix the receiving endpoint first. Use [`POST /webhooks/{id}/test`](/api-reference/webhooks/test) to confirm it accepts signed requests. Then call [Resume Webhook](/api-reference/webhooks/resume) to start sending waiting and rejected deliveries at once. Without it, a rejected delivery can wait up to 7 days.",
  ...
  "Xquik has not sent this delivery yet.",
  ...
  "Xquik retries with exponential backoff until your endpoint returns `2xx`.",
  "Retries never stop on their own.",
  '<Card title="exhausted" icon="pause">',
  "You paused or deleted the webhook while this delivery waited.",
] as const;
...
const REQUIRED_WEBHOOK_OVERVIEW_SNIPPETS = [
  ...
  "Return `2xx` within 10 seconds for every delivery, even one you skip. Any other status, a redirect, a timeout, or a network error is a failure. `410 Gone` is a normal failure. Xquik retries a failed delivery until your endpoint returns `2xx`. There is no attempt limit.",
  "Return `2xx` for every event you skip. Xquik keeps retrying a rejected delivery. While it fails, new events can arrive up to 15 minutes late.",
  "If a new event arrives within 2 seconds of each failure, a rejected delivery waits longer:",
  "The wait stops growing at 7 days. Retries can arrive a little later than listed. When no new event arrives within 2 seconds of a failure, Xquik retries the rejected delivery at the next check. Checks run up to 15 minutes apart.",
  "It retries 1 waiting delivery at a time to check the endpoint. This is a real delivery, not a `webhook.test`. Checks start 2 seconds apart and slow to 1 every 15 minutes.",
  "`needs_attention` means 200 failed checks in a row.",
  ...
  "Store `nextCursor` after every event page. Reprocess only events your receiver never stored. Skip repeats by `streamEventId`, since Xquik also resends failed deliveries. Events from a pause may never reach the webhook, so page them here. Scope event pages with `monitorId` for account monitors or `keywordMonitorId` for keyword monitors when the source is known.",
  "Type `string`. Webhook delivery ID. Every retry reuses it.",
  ...
  "Act on `failed`, wait on `pending`, and ignore",
  "Join `streamEventId` to [Get Event](/api-reference/events/get)",
] as const;
...
const REQUIRED_ARCHITECTURE_LIMITATION_SNIPPETS = [
  ...
  "Webhook deliveries have no attempt limit. Xquik retries each failure until it succeeds. Only you can pause or delete a webhook. Resume within 30 days of the event to receive held deliveries.",
...
const RETIRED_WEBHOOK_SNIPPETS = [
  "exhausts",
  "up to 10 attempts",
  "10-attempt",
  "Attempt 10",
  ...
  "Delivery resumes only after",
  "before delivery resumes",
  "needs attention before resuming",
  "needs attention before new deliveries continue",
  "Fix the receiver before resuming",
  "Record the recovery threshold",
  "used every retry attempt",
  "used all retry attempts",
  "all retries failed",
  "Page on `exhausted`",
  "page on `exhausted`",
  "retry until delivered or exhausted",
  "After the last retry, the status changes to",
  "makes no further attempts",
  "Final attempt. If it fails",
  "should stop retrying that delivery",
  "marks the delivery `exhausted` immediately",
  "or at the retry cap",
  "exhaust all attempts",
  "60 seconds for each attempt",
  "receiver_returned_410",
...
const REQUIRED_DM_HISTORY_API_SNIPPETS = [
  ...
  "Get DM history",
  "GET /x/dm/{userId}/history",
  "Read Twitter DMs between a connected account and one user.",
  "Twitter DM API",
  "Twitter API DM",
  "Twitter DM history API",
  "X DM history API",
  "This route reads the DM history between a connected account and one user.",
  "Pass the connected account through `account`.",
...
const REQUIRED_SEND_DM_API_SNIPPETS = [
  ...
  'description: "Send text or one media attachment as a direct message from a connected X account.',
  "Twitter DM API",
  "Twitter API DM",
  "Twitter API send DM",
  "messageId",
...
const REQUIRED_WORKFLOW_OVERVIEW_SNIPPETS = [
  ...
  "Account and keyword monitors both check every 1 second while active. Both can deliver events to the same signed webhooks.",
  "`deliveryId`, `streamEventId`, `eventType`, and tweet data",
  "Store the one-time `secret` returned by `POST /webhooks`. Verify `X-Xquik-Signature` against the raw body before accepting the event.",
  ...
  "21 credits per active monitor-hour. The rate includes webhook delivery",
...
const REQUIRED_BRAND_MONITORING_WORKFLOW_SNIPPETS = [
  ...
  "`deliveryId`, `streamEventId`, `eventType`, `occurredAt`,",
...
const REQUIRED_NO_CODE_WORKFLOW_HANDOFF_SNIPPETS = [
  ...
  '"n8n Twitter"',
  '"n8n Twitter node"',
  "This no code API integration pattern keeps cursors and event IDs.",
  ...
  "There is no attempt limit. Return `2xx` for events your workflow skips. Webhook health exposes `deliveryStatus`, `consecutiveFailures`, and `failureHardCap`. `needs_attention` does not stop retries.",
...
const REQUIRED_ZAPIER_ALTERNATIVE_SNIPPETS = [
  ...
  "Task tiers run from 100 tasks/month through custom task limits",
  "MCP tool calls use 2 tasks from the plan quota",
...
const REQUIRED_PIPEDREAM_GUIDE_SNIPPETS = [
  ...
  "For Pipedream data stores, map production `deliveryId` to `delivery_id` for receiver retry deduplication. Map `streamEventId` to `stream_event_id` when one monitor event should process once across endpoint changes.",
...
const REQUIRED_PREFECT_GUIDE_SNIPPETS = [
  ...
  "This Prefect Python library gives you typed credentials and 6 read-only tasks.",
  ...
  "provides 6 asynchronous Prefect tasks",
  '"prefect==3.4.25"',
...
const REQUIRED_COMPOSIO_MIGRATION_SNIPPETS = [
  ...
  "Can operators run tested rollback steps?",
...
const REQUIRED_LANGCHAIN_GUIDE_SNIPPETS = [
  ...
  "It keeps tweet IDs, timestamps, cursors, and route errors as typed values.",
  ...
  "Keep exact error messages for operators.",
...
const REQUIRED_N8N_GUIDE_SNIPPETS = [
  ...
  "Handle each of these response classes:",
  ...
  "For storage rows, map production `deliveryId` to `delivery_id` for receiver retry deduplication. Map `streamEventId` to `stream_event_id` when one monitor event should process once across endpoint changes.",
  '<Card title="Webhook event deduplication" icon="fingerprint">',
  "Store `deliveryId` for receiver retry deduplication. Store `streamEventId` when one monitor event should process once across endpoint changes.",
  '<Card title="Stored event replay" icon="activity">',
...
const REQUIRED_MAKE_GUIDE_SNIPPETS = [
  ...
  "For Make storage rows, map production `deliveryId` to `delivery_id` for receiver retry deduplication. Map `streamEventId` to `stream_event_id` when one monitor event should process once across endpoint changes.",
  '<Card title="Extraction jobs" icon="database">',
...
const REQUIRED_PHANTOMBUSTER_ALTERNATIVE_SNIPPETS = [
  ...
  "Plans differ by automation slots, monthly execution time, email credits, AI credits, and URL finder credits",
  "data extraction across 15+ platforms, 100+ automations and workflows, scheduled auto-refresh, and export limits",
  "CSV combines results from all runs so far, and JSON covers the most recent run. Free plan and Free Trial exports stop at 10 rows",
...
const REQUIRED_TWEETDECK_ALTERNATIVE_SNIPPETS = [
  ...
  "full post composer, scheduled posts, advanced search, top or latest post order, Decks",
  ...
  "X Pro help lists a full post composer, scheduled posts, advanced search, top or latest post order, Decks, a column creator, video docking, and account switching.",
...
const REQUIRED_ZERNIO_ALTERNATIVE_SNIPPETS = [
  ...
  "Accounts 3-10 cost USD 6/account/month, accounts 11-100 cost USD 3/account/month, and accounts 101-2,000 cost USD 1/account/month",
...
const REQUIRED_TWSCRAPE_ALTERNATIVE_SNIPPETS = [
  ...
  "Search and GraphQL Twitter APIs, and async/await functions that can run multiple scrapers in parallel",
  ...
  "They cover tweet details, retweeters, tweet replies, and user lookup by login or ID",
  "followers, verified followers, subscriptions, user tweets, user replies, and user media",
...
const REQUIRED_SPRINKLR_ALTERNATIVE_SNIPPETS = [
  ...
  "content planning, a digital asset manager, an editorial calendar, an omnichannel publisher, and an engagement dashboard",
  "real-time listening across 30+ social and digital channels, visual brand mentions, sentiment analysis, emotion analysis, entity identification, and text classification",
...
describe("repository discovery", (): void => {
  ...
  it.each([
    ...
    {
      name: "keeps retired webhook claims out of public Markdown",
      files: (): readonly string[] => listPublicMarkdownFiles(),
      snippets: RETIRED_WEBHOOK_SNIPPETS,
      issue: (snippet: string): string => `Public Markdown repeats retired claim "${snippet}".`,
    },
    {
    ...
    const findings = files().flatMap((file): DiscoveryFinding[] => {
      const source = normalizeEvidence(readFileSync(file, "utf8"));
      return snippets
        .filter((snippet) => source.includes(normalizeEvidence(snippet)))
