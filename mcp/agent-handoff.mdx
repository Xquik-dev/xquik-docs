...
<CardGroup cols={2}>
  ...
  <Card title="API MCP" icon="terminal">
    ...
    expose exactly 30 eligible GET routes. Current SDKs negotiate MCP
...
An active guest key gives `search` and `execute` a 30-operation read-only catalog. The `docs` tool remains available. Every route appears in the [guest paid-read inventory](/guides/guest-wallets#eligible-paid-read-routes). Batch `GET /api/v1/x/tweets` accepts up to 100 tweet IDs.
