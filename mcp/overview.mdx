...
The MCP server uses a **code-execution sandbox model**. Instead of 18+ discrete tools with fixed input schemas, the server exposes 2 tools:
...
The previous MCP server (v1) with 18 discrete tools is still available at `https://xquik.com/mcp/v1`. It uses a traditional tool-call model with structured input schemas instead of the code-execution sandbox.

If your integration depends on v1 tool names (`search-tweets`, `monitors`, `events`, etc.), point your MCP client to `/mcp/v1`. All new integrations should use the default v2 server at `/mcp`.
...
Works with 40+ AI coding agents including Claude Code, Cursor, GitHub Copilot, Codex, Windsurf, VS Code, Gemini CLI, and more. The skill covers MCP tools and 77 REST API endpoints.
