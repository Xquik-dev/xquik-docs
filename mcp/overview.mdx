...
| **Capabilities** | 18 tools (same core capabilities) | 38+ endpoints (finer control) |
| **Unique to REST** | - | API key management, webhook PATCH/deliveries, extraction export (file download), account locale update, save-style PUT endpoint |
...
|------|-------------|------|
| `monitors` | Manage monitored X accounts (actions: list, add, remove) | Free |
| `events` | Retrieve activity from monitored accounts (list with filters, or get single by ID) | Free |
| `webhooks` | Manage webhook endpoints (actions: list, add, remove, test) | Free |
| `search-tweets` | Search X for real-time tweets by keyword, hashtag, or user | Metered |
| `get-user-info` | Get X user profile: name, bio, follower/following count | Metered |
| `lookup-tweet` | Get full tweet details and engagement metrics | Metered |
| `check-follow` | Check follow relationship between two X accounts | Metered |
| `download-media` | Download images, videos, GIFs from one or more tweets | Metered |
| `extractions` | Bulk data extraction with 20 tool types (actions: estimate, run, list, get) | Metered (run) |
| `draws` | Giveaway draws from tweet replies (actions: run, list, get) | Metered (run) |
| `get-trends` | Trending topics across 12 regions | Free |
| `get-radar` | Trending topics & news from 6 sources (FREE for all subscribers) | Free |
| `compose-tweet` | Compose, refine, and score tweets using X algorithm data (steps: compose, refine, score) | Free |
| `styles` | Manage tweet style profiles (actions: analyze, get, list, delete, save, compare, analyze-performance) | Mixed |
| `drafts` | Manage tweet drafts (actions: save, list, get, delete) | Free |
| `get-account` | Check subscription status and usage | Free |
| `subscribe` | Get subscription checkout or management link | Free |
| `set-x-identity` | Link your X username to your Xquik account | Free |
...
Works with 40+ AI coding agents including Claude Code, Cursor, GitHub Copilot, Codex, Windsurf, VS Code, Gemini CLI, and more. The skill covers all 18 MCP tools and 38+ REST API endpoints.
