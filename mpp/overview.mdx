...
The [Machine Payments Protocol](https://mpp.dev) (MPP) is an open standard for machine-to-machine payments over HTTP. Xquik implements MPP on 16 X-API read-only endpoints, allowing you to pay per call without creating an account or subscribing.
...
| **Available endpoints** | 16 X-API read-only | All endpoints |
