...
<Note>
For charge-intent payments, the `Payment-Receipt` header is included on every settled response, even if the upstream lookup returned 404 or 5xx. Tempo settlement is irreversible once broadcast, so paying clients always receive the receipt to reconcile the on-chain transaction with the server trace.
</Note>
