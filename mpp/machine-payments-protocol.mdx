---
title: "Machine Payments Protocol for X API billing"
sidebarTitle: "MPP"
description: "Use HTTP 402 and Tempo USDC for anonymous fixed-price tweet, user, follow-check, article, trend, and community reads. Includes payment and receipt examples."
keywords:
...
You need no account, API key, or subscription. The payment credential authenticates the retried request.
...
An accountless guest wallet covers 30 prepaid GET routes, including these 7. See the [guest paid-read route inventory](/guides/guest-wallets#eligible-paid-read-routes). Direct MPP payment credentials and guest keys cannot access monitors, webhooks, extractions, draws, writes, account routes, or account credential routes. API MCP cannot execute guest wallet credential routes.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>USD 10 to 250 hosted checkout</td>
...
<CardGroup cols={2}>
  ...
  <Card title="Guest wallets" icon="wallet-cards" href="/guides/guest-wallets">
    ...
    Prepay 30 accountless reads with one scoped API key.
