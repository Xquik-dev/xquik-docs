...
Use [wallet.tempo.xyz/welcome](https://wallet.tempo.xyz/welcome) as a hosted UI to buy & send USDC. Run `mppx account create` to create a local account, copy its address, send USDC from the web wallet to that address, then run `mppx account export` to get the hex key.
